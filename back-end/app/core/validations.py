...
def validar_nome(nome: str) -> str:
    """
    Valida nomes de pessoas e entidades (Autores, Editoras, Usuários).
    Deve conter no mínimo 3 caracteres, não possuir números e não ter
    caracteres especiais nos extremos.
    """
    nome = nome.strip()
...
def validar_nome_livro(nome_livro: str) -> str:
    """
    Valida nomes de livros.
    Deve conter entre 2 e 255 caracteres, começar e terminar com caractere
    alfanumérico e não possuir espaços duplos.
    """
    ...
    if len(nome_livro) > 255:
        raise ValueError("Nome do livro deve ter no máximo 255 caracteres")

    if "  " in nome_livro:
        raise ValueError("Nome do livro não pode conter espaços duplos")

    if not nome_livro[0].isalnum():
        raise ValueError("Nome do livro deve começar com letra ou número")

    if not nome_livro[-1].isalnum():
        raise ValueError("Nome do livro deve terminar com letra ou número")
...
def validar_email(email: str) -> str:
    """
    Valida o formato e estrutura de e-mails, forçando o domínio .com.
    """
    email = email.strip()
...
def validar_endereco(endereco: str) -> str:
    """
    Valida o tamanho e a formatação estrutural do endereço.
    """
    ...
    if not endereco[0].isalnum() or not endereco[-1].isalnum():
        raise ValueError("Endereço inválido. Deve começar e terminar com letra ou número")
...
def validar_telefone(telefone: str) -> str:
    """
    Valida e formata o telefone para o padrão brasileiro: (DD) NNNN-NNNN ou (DD) NNNNN-NNNN.
    """
